...
export default function DiscordUser({
  ...
  children,
  disabled = false,
}: PropsWithChildren<DiscordUserProps>) {
  // state
  ...
  const {
    ...
    mutate,
  } = useFetchDiscordUser({ address }, shouldFetchDiscordUser);
  ...
  if (discordLoading) {
    ...
    if (discordUser?.avatar) {
      ...
      if (disabled) {
        return <p className="text-red-500">Discord not connected</p>;
      } else {
        return <DiscordConnectButton />;
      }
    }
  }
}
