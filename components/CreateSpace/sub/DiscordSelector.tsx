...
export default function DiscordSelector({
  ...
  const { data: channels, trigger: discordChannelsTrigger } =
    useFetchDiscordChannels(
      { guildId: selectedGuild?.id || "" },
      router.isReady,
    );
  const { data: botIsMember, trigger: isBotMemberTrigger } =
    useIsBotMemberOfGuild(
      { guildId: selectedGuild?.id },
      router.isReady && selectedGuild !== null,
    );
  const { data: roles, trigger: discordRolesTrigger } =
    useFetchDiscordGuildRoles({ guildId: selectedGuild?.id || "" });

  const resetRolesAndChannels = () => {
    setSelectedProposalChannel({
      name: "-",
      id: null,
    } as unknown as DiscordChannel);
    setSelectedAlertRole({ name: "-", id: null } as unknown as DiscordRole);
    setSelectedAlertChannel({
      name: "-",
      id: null,
    } as unknown as DiscordChannel);
  };
