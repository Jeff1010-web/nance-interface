...
import {
  ...
  useDelegated,
  useSetDelegate,
} from "@/utils/hooks/snapshot/Delegations";
...
const fetcher: Fetcher<
  ProfileResponse,
  VoterProfileProps & { url: string }
> = async ({ url, voter, space, proposal }) => {
  const res = await fetch(
    url + new URLSearchParams({ voter, space, proposal }),
  );
...
export default function VoterProfile({
  ...
  const { data, error, isLoading } = useSWR(
    shouldFetch ? { url: "/api/profile?", voter, space, proposal } : null,
    fetcher,
  );
  ...
  if (error) {
    ...
    // skeleton loader
    return (
      <div className="mt-1 flex animate-pulse items-center justify-between space-x-2">
        {/* Avatar */}
    ...
    return (
      <div className="mt-1 flex items-center justify-between space-x-2">
        ...
        <div className="flex space-x-1">
          ...
          <span>{1 + data.delegators.length ?? 0}</span>
...
const BUTTON_STYLE =
  "w-fit m-2 inline-flex items-center justify-center rounded-md border border-transparent bg-blue-600 px-2 py-1 text-sm font-medium disabled:text-black text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-300";

function DelegateActions({ delegate }: { delegate: string }) {
  const { address, isConnecting, isDisconnected } = useAccount();
  const { openConnectModal } = useConnectModal();
  const { data: delegatedAddress } = useDelegated(
    "jbdao.eth",
  ...
  if (isConnecting) {
    ...
    return (
      <button onClick={() => openConnectModal?.()} className={BUTTON_STYLE}>
        Connect Wallet
      </button>
    );
...
function SetDelegateButton({ delegate }: { delegate: string }) {
  const { data, isLoading, isSuccess, write, error } = useSetDelegate(
    "jbdao.eth",
    delegate,
  );
  ...
  return (
    <button
      disabled={!write || error !== null || isLoading}
      onClick={() => write?.()}
...
function ClearDelegateButton() {
  const { data, isLoading, isSuccess, write, error } =
    useClearDelegate("jbdao.eth");
