...
import { useProposalsExtendedOf, useVotedProposalIds } from "../../hooks/Proposal";
...
export default function Snapshot() {
    ...
    const { loading: votedLoading, data: votedIds } = useVotedProposalIds(space, hide && isConnected ? address : null)
        ...
        <Layout
            ...
            <div className="flex my-6 flex-col">
                ...
                <div id="space-operator" className="flex justify-center gap-x-3 pt-3">
                    ...
                    <button id="batch-vote-btn" className="px-4 py-2 font-semibold text-sm bg-amber-200 hover:bg-amber-300 rounded-xl shadow-sm">Batch Vote</button>
                </div>
                <div className="flex flex-row flex-wrap pt-4 mx-4 px-20 justify-center">
                    ...
                    {!loading && (
                        proposalsData.map(proposal => (
                            <a href={`https://snapshot.org/#/${space}/proposal/${proposal.id}`} target="_blank" rel="noreferrer" className={`${(votedIds.includes(proposal.id) && hide) ? "hidden" : "block"} border-2 rounded-xl m-3 p-3 hover:border-slate-800 transition-colors max-w-lg`}>
                                <h3 className="text-xl font-semibold">{proposal.title}</h3>
                                <div className="flex flex-row space-x-1 items-center">
                                    <span>Scores: {proposal.scores_total}</span>
                                    <span>Votes: {proposal.votes}</span>
                                </div>
                                <div className="flex flex-row space-x-1 pb-2">
                                    {Object.keys(proposal.voteByChoice).map(choice => (
                                        <span>{choice}: {proposal.voteByChoice[choice]}</span>
                                    ))}
                                </div>
